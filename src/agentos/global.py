...
class AgentDatabase:
    ...
    async def get_document(self, doc_id):
        async with aiosqlite.connect(self.db_file) as db:
            async with db.execute(
                "SELECT doc_content FROM Documents WHERE doc_id = ?", (doc_id,)
            ) as cursor:
    ...
    async def store_document(self, doc_type, doc_content):
        async with aiosqlite.connect(self.db_file) as db:
            cursor = await db.execute(
                "INSERT INTO Documents (doc_type, doc_content) VALUES (?, ?)",
    ...
    async def get_task(self, task_id):
        async with aiosqlite.connect(self.db_file) as db:
            async with db.execute(
                "SELECT user_id, task_type, task_description FROM Tasks WHERE task_id = ?",
                (task_id,),
            ) as cursor:
                row = await cursor.fetchone()
    ...
    async def update_task_type(self, task_id, task_type):
        async with aiosqlite.connect(self.db_file) as db:
            await db.execute(
                "UPDATE Tasks SET task_type = ? WHERE task_id = ?", (task_type, task_id)
            )
            await db.commit()
            return True

    async def update_task_description(
        self, task_id, new_global_epoch, new_regional_epoch, new_description
    ):
        async with aiosqlite.connect(self.db_file) as db:
            async with db.execute(
                "SELECT global_epoch, regional_epoch FROM Tasks WHERE task_id = ?",
                (task_id,),
            ) as cursor:
                row = await cursor.fetchone()
                if not row:
                ...
                old_g, old_r = row
                if (
                    (new_global_epoch >= old_g) and (new_regional_epoch > old_g)
                ):  # I'm not sure what do u mean by t2 (new) > t1 (old), so I assumed epoch global and regional
                    await db.execute(
                        "UPDATE Tasks SET global_epoch = ?, regional_epoch = ?, task_description = ? WHERE task_id = ?",
                        (
                            new_global_epoch,
                            new_regional_epoch,
                            new_description,
    ...
    async def save_progress(
        self, task_id, node_id, result, global_epoch, regional_epoch
    ):  # I think you miss send global_epoch, regional_epoch in the github explaination so I assumed that we need them to compare
        async with aiosqlite.connect(self.db_file) as db:
            ...
            async with db.execute(
                ...
                should_update = (not row) or (
                    global_epoch >= row[0] and regional_epoch > row[0]
                )

            if should_update:
                await db.execute(
                    "INSERT OR REPLACE INTO Progress (task_id, node_id, global_epoch, regional_epoch, result)VALUES (?, ?, ?, ?, ?)",
    ...
    async def get_progress(self, task_id, node_id):
        async with aiosqlite.connect(self.db_file) as db:
            await db.execute("PRAGMA foreign_keys = ON")
            async with db.execute(
                "SELECT task_id, node_id, global_epoch, regional_epoch, result FROM Progress WHERE task_id = ? AND node_id = ?",
                (task_id, node_id),
            ) as cursor:
    ...
    async def get_progress_list(self, task_id):
        async with aiosqlite.connect(self.db_file) as db:
            await db.execute("PRAGMA foreign_keys = ON")
            async with db.execute(
                " SELECT task_id, node_id, global_epoch, regional_epoch, result FROM Progress WHERE task_id = ?",
                (task_id,),
            ) as cursor:
    ...
    async def delete_progress(self, task_id, node_id):
        async with aiosqlite.connect(self.db_file) as db:
            await db.execute("PRAGMA foreign_keys = ON")
            await db.execute(
                "DELETE FROM Progress WHERE task_id = ? AND node_id = ?",
                (task_id, node_id),
            )
            await db.commit()
            return True
